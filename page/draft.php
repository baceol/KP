<?php if ($_SESSION['session_user']->role_name == 'Ketua Divisi Sekretaris' || $_SESSION['session_user']->role_name == 'Anggota Divisi Sekretaris') { ?>
    <div class="row ms-auto mb-3">
        <div class="col-md-4">
            <button type="button" class="btn btn-primary" onclick="insertDraft()">Unggah</button>
        </div>
    </div>
<?php } ?>
        ...
        foreach ($result as $document) {
            ...
            <tr>
                ...
                <td class="col-3 text-end">
                    <?php
                    if (!empty($document->tbdoc_storage)) {
                    ...
                    if ($_SESSION['session_user']->role_name == 'Ketua Divisi Sekretaris' || $_SESSION['session_user']->role_name == 'Anggota Divisi Sekretaris') {
                        echo '<button type="button" class="btn btn-primary ms-2" title="ubah file" aria-label="edit" onclick="editDocument(' . $document->tbdoc_id . ')"><i class="bi bi-pencil-square"></i></button>';
                        echo '<button type="button" class="btn btn-primary ms-2" title="publifikasi file" aria-label="publish" onclick="publishDocument(' . $document->tbdoc_id . ')"><i class="bi bi-check-square-fill"></i></button>';
                    }
                    ?>
                </td>
            </tr>
